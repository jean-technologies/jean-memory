...
The primary way to retrieve context from Jean Memory is with the `getContext` function. It provides predictable performance tiers, allowing you to choose the right balance of speed and quality for your application.

```typescript
const { getContext } = useJean();

// Fast: Get a quick, reliable result in 1-2 seconds.
const fastContext = await getContext("What was my recent idea about AI?", { mode: 'fast' });

// Deep: Get a comprehensive, synthesized analysis in ~30 seconds.
const deepContext = await getContext("Analyze my writing style across my recent essays.", { mode: 'deep' });

// Balanced (Default): Uses the standard intelligent orchestration.
const balancedContext = await getContext("What are my current project goals?");
```

### Performance Tiers

-   **`fast`**: Uses a direct `search_memory` call. Ideal for real-time applications where speed is critical.
-   **`deep`**: Uses the powerful `deep_memory_query` for in-depth analysis. Best for asynchronous tasks or when the user has explicitly requested a deep dive.
-   **`balanced`** (Default): Uses the standard, AI-driven orchestration to determine the best context. Performance is variable.
