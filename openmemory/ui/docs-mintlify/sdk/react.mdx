...
This code shows how to bypass the main `getContext` orchestration and use the lower-level memory tools directly. You can explicitly add new memories with `tools.add_memory` or perform a targeted vector search for specific information with `tools.search_memory`. This is useful when you need precise, deterministic control over the user's memory.
