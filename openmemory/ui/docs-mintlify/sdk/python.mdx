...
This code illustrates the two ways to interact with the Jean Memory API. The first line shows the primary, orchestrated approach using `get_context`, which handles the complex logic of context engineering for you. The subsequent lines show how to use the `tools` namespace for direct, deterministic control, allowing you to explicitly add or search for memories, bypassing the higher-level orchestration.
